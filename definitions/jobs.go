...
func (set *Set) PreProcess(do *Do) {
	log.Warn("We have hit Set's PreProcess!!!")
	set.Value, err = stringPreProcess(set.Value, do)
}

func (set *Set) Execute(do *Do) (*JobResults, error) {
	log.Warn("We have hit the execution phase!")
	var JobResults someResults
	someResults.JobResult = "Hello"
	someResults.JobVars["Hello"] = "World!"
	return someResults, nil
}
